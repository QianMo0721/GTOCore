...
public final class FloatInputWidget extends WidgetGroup {
    ...
    private void buildUI() {
        ...
        textField = new TextFieldWidget(buttonWidth + 2, 0, textFieldWidth, 20, () -> toText(valueSupplier.get()), stringValue -> setValue(clamp(Float.parseFloat(stringValue))));
    ...
    private void changeValue(ClickData cd, float multiplier) {
        if (!cd.isRemote) {
            ...
            setValue(clamp(valueSupplier.get() + amount * multiplier));
        }
    }
    ...
    private void updateTextFieldRange() {
        ...
        setValue(clamp(valueSupplier.get()));
    }
}

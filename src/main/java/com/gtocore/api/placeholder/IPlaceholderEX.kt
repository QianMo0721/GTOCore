 ...
 *
 * @param T 占位符所代表的目标对象的类型。
 * @param C 获取当前目标时所需的上下文对象的类型。
 * @param S 携带占位符配置信息的源对象类型 (例如 ItemStack)。
 */
interface IPlaceholderEXKT<T, S, C> {
     ...
     */
    fun getTargetLists(source: S): List<List<T>>
     ...
     */
    fun getCurrentTarget(source: S, context: C): T?
}
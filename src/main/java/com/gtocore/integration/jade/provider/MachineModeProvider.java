        ...
        if (serverData.contains("RecipeTypes") && serverData.contains("CurrentRecipeType")) {
            ...
            if (blockAccessor.showDetails()) {
                ...
                for (int i = 0; i < recipeTypesTagList.size(); i++) {
                    ResourceLocation recipeType = RLUtils.parse(recipeTypesTagList.getString(i));
                ...
                ResourceLocation recipeType = RLUtils.parse(
        ...
        if (blockAccessor.getBlockEntity() instanceof MetaMachineBlockEntity blockEntity) {
            ...
            if (recipeTypes != null && recipeTypes.length > 1) {
                if (blockEntity.getMetaMachine() instanceof IRecipeLogicMachine recipeLogicMachine) {
                    ...
                    if (recipeLogicMachine.getRecipeLogic().isWorking()) {
                        var recipe = recipeLogicMachine.getRecipeLogic().getLastOriginRecipe();
                        if (recipe != null && recipe.recipeType != null && recipe.recipeType != GTRecipeTypes.DUMMY_RECIPES) {
